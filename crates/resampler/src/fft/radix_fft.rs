...
impl<D> RadixFFT<D> {
    ...
    pub fn new(factors: Vec<Radix>) -> Self {
        ...
        #[cfg(target_arch = "x86_64")]
        let stockham_autosort_fn: StockhamAutosortFn =
            if is_x86_feature_detected!("avx") && is_x86_feature_detected!("fma") {
    ...
    fn detect_simd_width() -> SimdWidth {
        #[cfg(target_arch = "x86_64")]
        let simd_width = if is_x86_feature_detected!("avx") && is_x86_feature_detected!("fma") {
            SimdWidth::Width4
        } else {
...
mod tests {
    ...
    fn test_dc_signal() {
        ...
        for &factor in SINGLE_STAGE_FACTORS {
            ...
            for (i, bin) in output.iter().enumerate().skip(1) {
                assert!(
                    bin.re.abs() < EPSILON,
    ...
    fn test_parseval() {
        ...
        for &factor in SINGLE_STAGE_FACTORS {
            ...
            if size % 2 == 0 {
                ...
                for bin in output.iter().take(size / 2).skip(1) {
                    freq_energy += 2.0 * (bin.re * bin.re + bin.im * bin.im);
                }
            } else {
                // For odd sizes, all bins except DC are counted twice.
                for bin in output.iter().take(size / 2 + 1).skip(1) {
                    freq_energy += 2.0 * (bin.re * bin.re + bin.im * bin.im);
                }
            }
    ...
    fn test_multiple_sizes() {
        ...
        for log_size in 2..10 {
            ...
            for sample in reconstructed.iter_mut().take(size) {
                *sample /= size as f32;
            }

            for (i, (inp, rec)) in input
                .iter()
            ...
            {
                assert!(
                    approx_eq(*inp, *rec, EPSILON * 10.0),
                    "Size {size} failed at index {i}: {inp} != {rec}",
                );
            }
        }
    }
    ...
    fn test_random_signals() {
        ...
        for &factor in SINGLE_STAGE_FACTORS {
                ...
                .take(size)
            {
                assert!(
                    approx_eq(*inp, *rec, EPSILON * 10.0),
                    "Factor {factor:?}: Random signal failed at index {i}: {inp} != {rec}",
                );
            }
        }
    }
    ...
    fn test_zero_signal() {
        ...
        for &factor in SINGLE_STAGE_FACTORS {
            ...
            for (i, bin) in output.iter().enumerate() {
                assert!(
                    approx_eq(bin.re, 0.0, EPSILON),
                    "Factor {factor:?}: Bin {i} re should be 0"
                );
                assert!(
                    approx_eq(bin.im, 0.0, EPSILON),
    ...
    fn test_inverse_scaling() {
        ...
        for &factor in SINGLE_STAGE_FACTORS {
            ...
            for (i, rec) in reconstructed.iter().enumerate().take(size) {
                assert!(
                    approx_eq(*rec, 1.0, EPSILON),
                    "Factor {factor:?}: Sample {i} = {rec}, expected 1.0",
                );
            }
        }
    }
    ...
    fn naive_dft(input: &[f32]) -> Vec<Complex32> {
        ...
        for (k, output_bin) in output.iter_mut().enumerate().take(n / 2 + 1) {
    ...
    fn test_vs_naive_dft() {
        ...
        for &factor in SINGLE_STAGE_FACTORS {
            ...
            for (i, (out, naive)) in output.iter().zip(naive_output.iter()).enumerate() {
                assert!(
                    approx_eq_complex(*out, *naive, EPSILON * size as f32),
    ...
    fn test_multi_stage_round_trip() {
        ...
        for &(factors, desc) in MULTI_STAGE_FACTORS {
                ...
                .take(size)
            {
                assert!(
                    approx_eq(*inp, *rec, EPSILON * 10.0),
                    "Config {desc} (size {size}) failed at index {i}: {inp} != {rec}",
                );
            }
        }
    }
