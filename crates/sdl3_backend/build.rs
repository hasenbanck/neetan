...
fn compile_spirv(shader_file: &Path, output_dir: &Path, modules_dir: &Path) -> bool {
...
fn compile_dxil(shader_file: &Path, output_dir: &Path, modules_dir: &Path) -> bool {
    let base_name = shader_file.file_stem().unwrap().to_str().unwrap();
    let output_file = output_dir.join(format!("{base_name}.dxil"));
...
fn compile_metallib(shader_file: &Path, output_dir: &Path, modules_dir: &Path) -> bool {
    let base_name = shader_file.file_stem().unwrap().to_str().unwrap();
    let output_file = output_dir.join(format!("{base_name}.metallib"));
...
fn run_slangc(mut cmd: Command) -> bool {
    match cmd.output() {
        Ok(result) => {
...
fn targets_for(target_os: &str) -> Vec<ShaderTarget> {
    match target_os {
        "windows" => vec![ShaderTarget::Dxil, ShaderTarget::Spirv],
        "macos" | "ios" => vec![ShaderTarget::Metallib],
        _ => vec![ShaderTarget::Spirv],
    }
}
...
fn compile_pass(
    ...
    match target {
        ShaderTarget::Spirv => compile_spirv(shader_file, output_dir, modules_dir),
        ShaderTarget::Dxil => compile_dxil(shader_file, output_dir, modules_dir),
        ShaderTarget::Metallib => compile_metallib(shader_file, output_dir, modules_dir),
    }
}
...
fn main() {
    ...
    let targets = targets_for(&target_os);
    ...
    for pass_file in &pass_files {
        ...
        for target in &targets {
            let success = compile_pass(*target, pass_file, &subdirectory, &modules_dir);
            if !success {
                had_error = true;
            }
        }
