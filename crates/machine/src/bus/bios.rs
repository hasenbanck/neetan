    ...
    fn hle_bootstrap(&mut self, cpu: &mut impl Cpu) {
        ...
        for hdd in 0..2usize {
            if self.sasi.drive_geometry(hdd).is_some() {
                ...
                let geometry = self.sasi.drive_geometry(hdd).unwrap();
                let pos = device::sasi::sector_position(drive_select, 0, 0, &geometry);
                let addr = device::sasi::buffer_address(0x1FC0, 0x0000);
                let xfer = device::sasi::transfer_size(0x0400);
                let result = {
                    let memory = &mut self.memory;
                    self.sasi.execute_read(hdd, xfer, pos, addr, |a, byte| {
