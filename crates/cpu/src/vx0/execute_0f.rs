...
#[inline(always)]
const fn bitfield_full_prefetch_queue_target(model: u8) -> usize {
    queue_size_for(model) - 1
}
...
impl<const MODEL: u8> VX0<MODEL> {
    ...
    fn bit1_prefetch_target(&self) -> usize {
        let entry_full = self.biu_instruction_entry_queue_len_for_timing() == queue_size_for(MODEL);
        let instruction_len = self.ip.wrapping_sub(self.prev_ip) as usize;
        if entry_full && instruction_len <= queue_size_for(MODEL) {
            2
        } else {
            1
        }
    ...
    fn bit1_write_prefetch_target(
        ...
        (target_queue_len + extra).min(queue_size_for(MODEL))
    }
    ...
    fn bitfield_register_count_prefetch_target(&self) -> usize {
        if self.biu_instruction_entry_queue_len_for_timing() == queue_size_for(MODEL) {
            bitfield_full_prefetch_queue_target(MODEL)
        } else {
    ...
    fn bitfield_ins_prefetch_before_memory(
        ...
        if zero_offset {
            while self.queue_len() < queue_size_for(MODEL) && self.queue_has_room_for_fetch() {
                self.biu_start_code_fetch_for_eu();
                self.clk(bus, 4);
                self.biu_complete_code_fetch_for_eu();
            }
    ...
    fn bitfield_insert_prefixed_or_cold(&self) -> bool {
        self.seg_prefix || self.biu_instruction_entry_queue_len_for_timing() < queue_size_for(MODEL)
    }
    ...
    fn bitfield_insert_post_read_prefetch(
        ...
        if immediate_count {
            let full_unprefixed = self.biu_instruction_entry_queue_len_for_timing()
    ...
    fn rol4(&mut self, bus: &mut impl common::Bus) {
        ...
        if modrm >= 0xC0 {
            ...
            let cycles = if self.biu_instruction_entry_queue_len_for_timing()
                == queue_size_for(MODEL)
                && !self.seg_prefix
            {
                13
