...
fn timer_a_configuration() {
    ...
    write_reg_y8950(&mut chip, 0x02, 0xFF);
    write_reg_y8950(&mut chip, 0x04, 0x01);
...
fn timer_b_configuration() {
    ...
    write_reg_y8950(&mut chip, 0x03, 0x80);
    write_reg_y8950(&mut chip, 0x04, 0x02);
...
fn timer_flag_cleared_by_reset_bit() {
    ...
    write_reg_y8950(&mut chip, 0x04, 0x80);
...
fn timer_irq_assert_and_deassert() {
    ...
    let irq_assert = events
        .iter()
        .find(|e| matches!(e, CallbackEventExt::UpdateIrq { asserted: true }));
    assert!(
        irq_assert.is_some(),
    ...
    let irq_deassert = events
        .iter()
        .find(|e| matches!(e, CallbackEventExt::UpdateIrq { asserted: false }));
    assert!(
...
fn both_timers_active() {
    ...
    write_reg_y8950(&mut chip, 0x02, 0xFF);
    write_reg_y8950(&mut chip, 0x03, 0x80);
    write_reg_y8950(&mut chip, 0x04, 0x03);
...
fn timer_a_period_varies_with_value() {
    let mut chip1 = Y8950::new(RecordingCallbacksY8950::new());
    chip1.reset();
    chip1.callbacks().take_events();

    write_reg_y8950(&mut chip1, 0x02, 0x00);
    write_reg_y8950(&mut chip1, 0x04, 0x01);
    ...
    let mut chip2 = Y8950::new(RecordingCallbacksY8950::new());
    chip2.reset();
    chip2.callbacks().take_events();

    write_reg_y8950(&mut chip2, 0x02, 0xFF);
    write_reg_y8950(&mut chip2, 0x04, 0x01);
    ...
    if let (Some(d1), Some(d2)) = (duration1, duration2) {
        ...
        assert!(
            events1
                .iter()
                .any(|e| matches!(e, CallbackEventExt::SetTimer { timer_id: 0, .. })),
