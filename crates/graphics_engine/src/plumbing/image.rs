...
/// A 2D image with `TRANSFER_DST | SAMPLED` usage, used as a destination for
/// `vkCmdCopyBufferToImage` and subsequent sampling.
///
/// Render passes transition the layout to `TRANSFER_DST_OPTIMAL` for the copy
/// and back to `SHADER_READ_ONLY_OPTIMAL` for sampling.
pub(crate) struct SampledTransferImage {
    handle: vk::Image,
    view: vk::ImageView,
    memory_block: Option<MemoryBlock>,
    context: Rc<Context>,
...
impl SampledTransferImage {
    /// Creates a new `TRANSFER_DST | SAMPLED` 2D image, transitioned to
    ...
    pub(crate) fn new(
        ...
        let handle = unsafe {
            context
                .device()
                .create_image(&image_create_info, None)
                .context("Failed to create sampled-transfer image")?
        };
        ...
        let memory_block = unsafe {
            match context.allocator().alloc(context.device(), request) {
                ...
                Err(error) => {
                    context.device().destroy_image(handle, None);
                    return Err(error)
                        .context("Failed to allocate memory for sampled-transfer image")?;
                }
            }
        };
        ...
        unsafe {
            if let Err(error) = context.device().bind_image_memory(
                ...
                *memory_block.memory(),
                ...
                return Err(error).context("Failed to bind memory for sampled-transfer image")?;
            }
        }
        ...
        let view = match unsafe { context.device().create_image_view(&view_create_info, None) } {
            ...
            Err(error) => {
                ...
                return Err(error).context("Failed to create sampled-transfer image view")?;
            }
        };
        ...
        if let Err(error) = layout_transitioner.transition_layout(
            ...
            return Err(error)
                .context("Failed to transition sampled-transfer image to initial layout")?;
        }
...
impl Drop for SampledTransferImage {
    fn drop(&mut self) {
        unsafe {
            self.context.device().destroy_image_view(self.view, None);
            self.context.device().destroy_image(self.handle, None);
